...
#line default
	...
	private void LogPathResults (Pathfinding.Path p) {
		...
		if (logPathResults == PathLog.InGame) {
			...
			//Furture path has been calculated and is complete
			//Debug.Log(debug);
		}
	}
	...
	public void ReturnPaths (bool timeSlice) {
		...
		Pathfinding.Path p = pathReturnStack.PopAll();

		if (pathReturnPop == null) {
			pathReturnPop = p;
		} else {
			Pathfinding.Path tail = pathReturnPop;
		...
		while (pathReturnPop != null) {
			...
			Pathfinding.Path prev = pathReturnPop;
	...
	private static void CalculatePathsThreaded (System.Object _threadInfo) {
		...
		try {
			...
			while (true) {
				...
				Pathfinding.Path p = astar.pathQueue.Pop();
