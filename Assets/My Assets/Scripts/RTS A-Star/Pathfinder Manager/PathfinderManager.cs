...
namespace AStar {
    ...
    public partial class PathfinderManager : MonoBehaviour {
        ...
        //Singleton
        public static PathfinderManager main;
        ...
        /*
        //Trys to start the queued process
        public void TryProcessNext() {
            if(!isProcessingPath) {   //If you are not alreadying processing
                if(pathRequestQueue.Count > 0) {    //If their is at least one queued request
                    currentPathRequest = pathRequestQueue.Dequeue();
                    isProcessingPath = true;
                    pathfinding.StartFindPath(currentPathRequest.pathStart, currentPathRequest.pathEnd);
                } else {
                    Debug.Log("No path requests are queued.");
                }
            } else {
                Debug.Log("Path process is already running.");
            }
        }
        */
...
Managers:
    PathfindingManager = PathRequestManager(1st part), Pathfinding(2nd part) -Partial classes

Componets on all Units:
    RTSGameObject/Unit - Links with Pathfinder [Requiers Pathfinder Component]
    Pathfinder - Universal - Links with PathfindingManager
